...
async function  Productslug({ params }) { 
 ...
 const hasDiscount = product.discount > 0 && product.offPrice && product.offPrice < product.price;
            ...
            <div className="bg-white rounded-lg shadow-xl overflow-hidden max-w-4xl w-full flex flex-col md:flex-row">
              ...
              <div className="flex justify-center items-center p-4">
                ...
                <div className="md:w-1/2 p-6 md:p-8 flex flex-col justify-between">
                    <div>
                        <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 mb-3">{product.title}</h1>
                        <p className="text-gray-600 text-sm mb-2">برند: <span className="font-semibold">{product.brand}</span></p>
                        <p className="text-gray-600 text-sm mb-4">دسته‌بندی: <span className="font-semibold">{product.category?.title || 'نامشخص'}</span></p>
                        ...
                        <div className="text-gray-700 mb-4">
                            <p>موجود در انبار: <span className="font-semibold">{product.countInStock > 0 ? `${product.countInStock} عدد` : 'ناموجود'}</span></p>
                            <p>امتیاز: <span className="font-semibold">{product.rating}</span> ({product.numReviews} نظر)</p>
                        </div>
                    </div>
                    ...
                    <div className="mt-4 text-xs text-gray-500">
                        <p>تاریخ انتشار: {new Date(product.createdAt).toLocaleDateString('fa-IR')}</p>
                    </div>
                </div>
            </div>
        </div>
